...
generate_identifier :: proc(ctx: ^generation.gen_context, node: ^ast.node, register_num: int, child_location: location, contains_allocations: bool) -> location
{
  ...
  if ast.is_member(node)
  {
    ...
    if ast.is_type(child_node)
    {
      ...
      case "max":
        ...
        {
        case "f32": return memory(get_literal_name(&ctx.root.children["[f32_literals]"], "f32_", fmt.aprintf("%f", math.max(f32))), 0)
        case "f64": return memory(get_literal_name(&ctx.root.children["[f64_literals]"], "f64_", fmt.aprintf("%f", math.max(f64))), 0)
      ...
      case "min":
        switch child_node.value
        {
        case "f32": return memory(get_literal_name(&ctx.root.children["[f32_literals]"], "f32_", fmt.aprintf("%f", math.min(f32))), 0)
        case "f64": return memory(get_literal_name(&ctx.root.children["[f64_literals]"], "f64_", fmt.aprintf("%f", math.min(f64))), 0)
      ...
      case "name":
        // TODO as static var?
        return memory(get_literal_name(&ctx.root.children["[string_literals]"], "string_", fmt.aprintf("\"%s\"", ast.type_name(child_node))), 0)
    ...
    child_type_node := child_node.data_type
      ...
      case .subscript:
        ...
        else
        {
          ...
          if len(node.value) == 1
          {
            index := ast.get_swizzle_index(rune(node.value[0]))
  ...
  if !memory_allocator && node.allocator != ctx.root.identifiers["stack"]
  {
    ...
    {
      _, declaration_path := ast.get_declaration(ctx.root, ctx.scope, node)
      ...
      name = ast.get_scope_name(path[:], true)
    }
