...
{
  buf: [8]byte
  return { .immediate, strings.clone(strconv.itoa(buf[:], value)), 0, "", 0 }
}

immediate_string :: proc(value: string) -> location
{
  return { .immediate, value, 0, "", 0 }
}

memory :: proc(address: string, offset: int) -> location
{
  return { .memory, address, offset, "", 0 }
}
...
register_named :: proc(name: string, type_node: ^ast.node) -> location
{
  ...
  case 1:
    if strings.ends_with(name, "x")
    {
      ...
      return { .register, strings.concatenate({ first_char, "l" }), 0, name, 0 }
    }
    else
    {
      return { .register, strings.concatenate({ name, "l" }), 0, name, 0 }
    }
  case 2:
    return { .register, name, 0, name, 0 }
  case 4:
    return { .register, strings.concatenate({ "e", name }), 0, name, 0 }
  case 8:
    return { .register, strings.concatenate({ "r", name }), 0, name, 0 }
  }
...
register_numbered :: proc(number: int, type_node: ^ast.node) -> location
{
  ...
  if float_type
  {
    ...
    return { .register, strings.concatenate({ "xmm", number_string }), 0, "", number }
  }
  ...
  {
  case 1:
    return { .register, strings.concatenate({ "r", number_string, "b" }), 0, "", number }
  case 2:
    return { .register, strings.concatenate({ "r", number_string, "w" }), 0, "", number }
  case 4:
    return { .register, strings.concatenate({ "r", number_string, "d" }), 0, "", number }
  case 8:
    return { .register, strings.concatenate({ "r", number_string }), 0, "", number }
  }
