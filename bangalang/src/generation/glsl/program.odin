...
generate_program :: proc(ctx: ^generation.gen_context, node: ^ast.node)
{
  ...
  param0_type_node := ast.get_type(param0_node)
  assert(param0_node.value == "index" && param0_type_node.value == "i32", "First param must be 'index: i32'");
  ...
  for &param_node, index in params_type_node.children[1:]
  {
    ...
    fmt.sbprintln(&ctx.output, "{")
    fmt.sbprintfln(&ctx.output, "  %s %s%s;", type_name(param_type_node), param_node.value, param_type_node.value == "[slice]" ? "[]" : "")
    fmt.sbprintln(&ctx.output, "};")
    fmt.sbprintln(&ctx.output, "")
  }
