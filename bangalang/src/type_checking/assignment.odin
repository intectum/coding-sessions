...
type_check_assignment :: proc(ctx: ^type_checking_context, node: ^ast.node) -> bool
{
  ...
  if procedure_definition
  {
    ...
    scope := ast.get_scope(ctx.program, ctx.scope.path)
  ...
  if declaration
  {
    _, memory_allocator := ast.coerce_type(lhs_node.allocator.data_type, ctx.program.identifiers["memory_allocator"])
    if memory_allocator
    {
      if len(node.children) > 1
      {
