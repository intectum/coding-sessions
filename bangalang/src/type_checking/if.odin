...
type_check_if :: proc(ctx: ^type_checking_context, node: ^ast.node) -> bool
{
  ...
  type_check_rhs_expression(ctx, expression_node, ctx.root.identifiers["bool"]) or_return
  ...
  {
  type_check_rhs_expression(ctx, node.children[child_index], ctx.root.identifiers["bool"]) or_return
  child_index += 1
