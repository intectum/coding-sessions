...
type_check_procedure :: proc(ctx: ^type_checking_context, node: ^ast.node) -> bool
{
  ...
  _, code_allocator := ast.coerce_type(lhs_node.allocator.data_type, ctx.root.identifiers["code_allocator"])
  ...
  if len(node.children) > 1
  {
    ...
    _, nil_allocator := ast.coerce_type(lhs_node.allocator.data_type, ctx.root.identifiers["nil_allocator"])
    if nil_allocator
    {
