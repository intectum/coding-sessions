...
type_check_call :: proc(node: ^ast.node, ctx: ^type_checking_context) -> bool
{
  ...
  {
    src.print_position_message(node.src_position, "Wrong number of parameters passed to procedure '%s' (expected %i, found %i)", procedure_node.value, len(params_type_node.children), len(node.children) - 1)
    return false
  }
  ...
  for child_index < len(node.children)
  {
    ...
    type_check_rhs_expression(param_node, ctx, ast.get_type(param_node_from_type)) or_return
  }
