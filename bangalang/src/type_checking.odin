...
type_check_if :: proc(node: ast_node, ctx: ^type_checking_context) -> (ok: bool = true)
{
    ...
    for child_index + 1 < len(node.children)
    {
        else_if_expression_ok := type_check_expression(node.children[child_index], ctx)
...
type_check_expression :: proc(node: ast_node, ctx: ^type_checking_context) -> (ok: bool = true)
{
