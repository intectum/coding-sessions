...
parse_assignment :: proc(ctx: ^parsing_context, stream: ^tokens.stream) -> (node: ^ast.node, ok: bool)
{
  node = ast.make_node({
  ...
  if !ast.is_member(lhs_node) && lhs_type_node != nil && (lhs_type_node.type == .kernel_type || lhs_type_node.type == .procedure_type)
  {
